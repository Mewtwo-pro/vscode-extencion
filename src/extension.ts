...
let myvar = 0;
let start_pos = new vscode.Position(1,2);
let sub_mode = 0
    ...
    async function ejecutarFaseEtiquetas(
        ...
        selectionInput.onDidChangeValue((value) => {
            ...
            if (mapa.has(letraEscrita)) {
                ...
                if (savedLine == -1){
                    editor.selection = new vscode.Selection(start_pos,pos)
                    select_mode = 1
                    myvar = 2
                }else {
...
const infoatajos = vscode.commands.registerCommand("myExtension.infoatajos", async () => {
    const info_atajos = ['add file Git (n),resetPath (s)' ,'filtrar (k)',
        ...
        'go new folder (z)','crear new folder (x)',
            ...
            input.onDidChangeValue(value => {
                if (value === "g"){
                    ...
                    if (editor){
                        ...
                        const inicioContenido = match ? match[0].length : 0;
                        editor.edit(editBuilder => {
                            editBuilder.delete(
                                new vscode.Range(
                                    new vscode.Position(position.line, inicioContenido),
                                    position
                                )
                            );
                        });
                    ...
                    const editor = vscode.window.activeTextEditor;
                    if (editor){
                        start_pos = editor.selection.active
                        vscode.commands.executeCommand('myExtension.flash')
                    }
                }
            });
...
const setopenfolder = vscode.commands.registerCommand("myExtension.setopenfolder", async () => {
        myvar = 0
        await vscode.commands.executeCommand('myExtension.gonewfolderinput');
});
...
const gonewfolderinput = vscode.commands.registerCommand(
    'myExtension.gonewfolderinput',
    async () => {
    ...
    if (text.endsWith(" ")) {
        ...
        try {
            ...
            const folders = items
                ...
                .map(item => item.name);
            if (folders.length === 0) {
                vscode.window.showInformationMessage('No hay subcarpetas en la ruta actual.');
                return;
            }
            vscode.window.showQuickPick(folders, {
                placeHolder: `Carpetas en: ${currentDir}`
            }).then(selectedFolder => {
                if (selectedFolder) {
                    const fullPath = path.join(currentDir, selectedFolder);
                    vscode.commands.executeCommand('myExtension.navigateFolder', fullPath);
                    vscode.window.showInformationMessage(`Seleccionaste: ${fullPath}`);
                }
            });
