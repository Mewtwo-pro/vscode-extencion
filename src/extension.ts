...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.window.registerTreeDataProvider('myCustomExplorer', provider);

    // Refrescar al cambiar de editor activo
    vscode.window.onDidChangeActiveTextEditor(() => {
        provider.refresh();
    });
// 2. Registra el comando de filtrar
context.subscriptions.push(
    vscode.commands.registerCommand('myExtension.filterFiles', async () => {
        const filter = await vscode.window.showInputBox({
            placeHolder: "Escribe para filtrar archivos...",
            prompt: "Filtrar archivos en la carpeta actual"
        });
...
const tresCommand = vscode.commands.registerCommand("myExtension.tresCommand", async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) return;
    if (myvar == 5){
       myvar = 0
...
const spaceKey = vscode.commands.registerCommand(
    ...
    const tresKey = vscode.commands.registerCommand(
        'myExtension.tresKey',
        () => {
            if (myvar == 5 && miInput ){
                myvar = 0
